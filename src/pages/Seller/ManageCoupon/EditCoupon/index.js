...
import { Button, Card, Col, DatePicker, Form, Image, Input, InputNumber, Row, Space, Switch, Table, Tooltip, Typography } from "antd";
import dayjs from "dayjs";
import locale from 'antd/es/date-picker/locale/vi_VN';
import { getUserId, regexPattern } from "~/utils";
import { COUPON_TYPE_SPECIFIC_PRODUCTS, RESPONSE_CODE_NOT_ACCEPT, RESPONSE_CODE_SUCCESS } from "~/constants";
import { useNavigate, useParams } from "react-router-dom";
import { NotificationContext } from "~/context/NotificationContext";
import { checkCouponCodeExist, editCouponSeller, getCouponSellerById } from "~/api/coupon";
import PopupSelectProduct from "~/components/Seller/Coupon/Add/Product/PopupSelectProduct";
...
function EditCoupon() {
        ...
        getCouponSellerById(getUserId(), couponId)
            .then((res) => {
                if (res.data.status.responseCode === RESPONSE_CODE_SUCCESS) {
                    ...
                    setLsProductApplied(res.data.result.productsApplied);
                }
            })
            .catch((err) => {
    ...
    const onFinish = ({ couponCode, couponName, startDate, endDate, priceDiscount, minTotalOrderValue, isPublic, quantity }) => {
        ...
        const data = {
            ...
            isPublic,
            quantity,
            typeId: coupon.couponTypeId,
        }
        if (coupon.couponTypeId === COUPON_TYPE_SPECIFIC_PRODUCTS) {
            data.productsApplied = lsProductApplied.map(v => v.productId);
        }
    ...
    return (<>
        {isOpenPopupSelectProduct && coupon?.couponTypeId === COUPON_TYPE_SPECIFIC_PRODUCTS && <PopupSelectProduct
