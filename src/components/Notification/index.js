...
import { SIGNAL_R_CHAT_HUB_RECEIVE_NOTIFICATION, SIGNAL_R_CHAT_HUB_RECEIVE_ALL_NOTIFICATION } from '~/constants';
...
function Notification() {
    ...
    useEffect(() => {
        ...
        connection.on(SIGNAL_R_CHAT_HUB_RECEIVE_ALL_NOTIFICATION, (res) => {
            const notifi = JSON.parse(res);
            setNotifications((prev) => [...notifi, ...prev]);
        });

        // Receive notifications from the server
        connection.on(SIGNAL_R_CHAT_HUB_RECEIVE_NOTIFICATION, (res) => {
            const notifi = JSON.parse(res);
